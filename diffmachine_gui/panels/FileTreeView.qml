...
Rectangle {
    ...
    function updateSelectAllFilesState() {
        if (!selectAllFilesCheckBox || !fileTreeView || !fileTreeView.model) {
            return
        }
        
        var selectableFiles = []
        
        function collectFiles(model, parentIndex, collected) {
            if (!model) return
                ...
                for (var i = 0; i < rowCount; i++) {
                    ...
                    if (filePath) {
                        ...
                        if (!isDir) {
                            ...
                            collectFiles(model, index, collected)
                        }
                    }
                }
            } catch (e) {
        ...
        collectFiles(fileTreeView.model, undefined, selectableFiles)
        
        if (selectableFiles.length === 0) {
            selectAllFilesCheckBox.internalUpdate = true
            selectAllFilesCheckBox.checkState = Qt.Unchecked
            selectAllFilesCheckBox.internalUpdate = false
            return
        }
        
        var allChecked = true
        var anyChecked = false
        for (var j = 0; j < selectableFiles.length; j++) {
            var path = String(selectableFiles[j])
        ...
        selectAllFilesCheckBox.internalUpdate = true
        selectAllFilesCheckBox.checkState = targetState
        selectAllFilesCheckBox.internalUpdate = false
    }
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Image {
                    ...
                    source: fileTreeViewRoot.theme.getIconPath("system-search.svg")
                ...
                MacTextField {
                    ...
                    onTextChanged: {
                        fileTreeViewRoot.searchFilter = text.toLowerCase()
                    }
                }
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                MacCheckBox {
                    ...
                    enabled: fileTreeViewRoot.showCheckboxes
                    ...
                    onClicked: {
                        if (!internalUpdate) {
                            ...
                            fileTreeViewRoot.selectAllFiles(shouldCheck)
                        }
                    }
                }
