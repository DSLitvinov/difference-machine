...
| `gc` | `enabled`, `reflog.expire.days`, `schedule.enabled`, `interval.day`, `schedule.hour`, `schedule.minute`, `last.run` | Shared with GUI Settings |
...
- `save_gc_config` — при смене GC prefs; `last.run` после `gc.run`
