...
**MVP v1 (full scope):** Sidebar (Project + History) + Content Preview (Project + History diff) + Content Info (Project) + multi-repo + 3-panel resize. **Branch UX:** GitHub Desktop — `currentBranch` единственная ветка; dropdown в History = checkout on select ([sidebar-history-view.md §2.6](./sidebar-history-view.md)). Канон API/events: [api-contract.md](./api-contract.md). Порядок реализации: [api-contract.md §7](./api-contract.md).
