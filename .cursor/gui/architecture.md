...
| Диалоги | [dialogs](./dialogs/architecture.md) | Модалки `Dialog / …`; файлы, restore, обслуживание репо |
| Состояния | [states](./states/architecture.md) | Режимы репозитория и UI; toast / баннеры / Damaged |
...
- `ok: false` — ошибка для toast / диалога; текст `error` можно показать пользователю, если он понятен, иначе нейтральное сообщение из спеки UI. Каталог поверхностей: [dialogs](./dialogs/architecture.md#ошибки), [states](./states/architecture.md#ошибки-и-уведомления).
...
`.DFM/` скрыт от `workdir.tree` / `workdir.entries`. Исключение: `workdir.open` может открыть `.DFM/tmp_review` (результат `compare.extract`). Пути из `.dfmignore` скрыты в entries/search/tree по умолчанию; **View ignored** передаёт `include_ignored`.
...
- Сканирование workdir, метаданные файла, thumbnail (картинка / кадр видео / `.blend` / текст), rename, удаление в корзину ОС, поиск, open, ignore / unignore, правка `.dfmignore`
...
| Рабочая копия | `status.get`, `workdir.*`, `index.add`, `index.drop` | Project: дерево, сетка default 48×48, dirty |
