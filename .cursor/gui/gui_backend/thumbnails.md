...
| `kind` | Поля | Когда |
|--------|------|--------|
| `image` | `mime`, `content_base64` | Превью готово: растр (картинка/кадр видео через ffmpeg или PNG из .blend) либо исходный SVG (`image/svg+xml`) |
...
Расширения: `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.exr`, `.tiff`, `.tif`, `.bmp`, `.svg`.

`.svg` не гоняется через ffmpeg: API отдаёт исходные байты с `mime: image/svg+xml`, GUI рисует их как `<img>` в webview. Нет ffmpeg на это не влияет. Файл больше 5 MiB (`maxThumbnailBytes`) → `placeholder`; полный кадр в Content View — `workdir.file` (до 64 MiB).

Остальные картинки: системный/бандленный ffmpeg → PNG в pipe. Нет ffmpeg или ошибка → `placeholder` (не падение всей сетки).

Дорого: каждый `Call` без кэша снова запускает ffmpeg. Disk-кэш GUI — только для этого `kind: image` с ffmpeg (не для blend и не для SVG, см. ниже).
...
Расширения (полный список в `isTextExt`): `.txt`, `.md`, `.json`, `.xml`, `.tsx`, `.ts`, `.js`, `.jsx`, `.go`, `.py`, `.rs`, `.css`, `.html`, `.yaml`, `.yml`, `.ini`, `.cfg`, `.sh`.

Показ: моноширинный фрагмент `text_preview` в тайле сетки и в File Info. Не подгружать весь файл вторым запросом, если thumbnail уже отдал preview. Файл больше 32 KiB — всё равно `kind: text` с префиксом.
...
| Не писать | text, placeholder, blend (OS/TEST уже на диске ОС), SVG (исходник рисует webview), blob ревизии |
...
tile visible
  → memory hit? показать
  → kind ожидания ffmpeg-image (растр или видео, не SVG) и disk hit? показать, прогреть memory
  → workdir.thumbnail
       image (ffmpeg) → memory + disk
       image (blend)  → memory only
       image (svg)    → memory only
       text           → memory
       placeholder    → memory (без ретрая)
```

Blend, SVG и текст диск-слой пропускают.
