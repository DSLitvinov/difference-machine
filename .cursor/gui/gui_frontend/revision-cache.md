...
| **Index / `staged_*`** | Живой индекс (`status.get`, `index.add`, `index.drop`) | Снимок VCS-store. Не LRU blob. Превью этих файлов — workdir + mtime, не hash коммита |
...
После `index.add` / `index.drop` / `commit.create` / watcher: обновить снимок `status.get`. Содержимое staged-файла в сетке — `workdir.thumbnail` + ключ с mtime.
