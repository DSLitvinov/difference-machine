...
| **View ignored** вкл. | тот же каталог, но `include_ignored: true`; ignored-файлы и папки с бейджем **i**. Default выкл. — пути из `.dfmignore` скрыты |
| Поиск | [folder-action](../components/items/folder-action.md): корень — весь репо; иначе префикс папки. Search тоже передаёт `include_ignored` |

После ignore при View ignored выкл. path исчезает из каталога: снять его с selection; открытый файл — вернуться в сетку папки.
