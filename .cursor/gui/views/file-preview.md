...
| File View - File Missing | [`6066:13219`](https://www.figma.com/design/qlwKiMPZblz96VSM2F3DlS/DFM-for-Cursor?node-id=6066-13219) | файл нет на диске (удалён / переименован / перемещён) | [File Info - Missing](../panels/file-info.md) 332 | File Expanded: [Content / View / Missing file](../components/items/content-view.md); More в хедере disabled |
...
Хедер центра: [Header File Action](../components/items/header-file-action.md) — More `⋯` открывает меню файла. Пункт меню выполняется сразу. Не combobox + Apply и не третья кнопка Save.
...
5. Данные left: `log.get` + `path`. Не `workdir.tree`.
6. Img в центре: полный файл через `workdir.file`, не `workdir.thumbnail`. Превью остаётся только в File Info. Ctrl/Cmd+wheel меняет масштаб картинки, без подписи zoom. Канон: [content-view](../components/items/content-view.md).
7. Text в центре: подсветка синтаксиса по расширению, без дополнительного chrome.
