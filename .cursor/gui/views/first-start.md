...
Title bar: `Difference Machine` без скобок — репозиторий ещё не открыт. После Create / Open — `Difference Machine ({name})` по [Header Window](../components/items/header-window.md).
