...
На холсте 25 фреймов с префиксом `View /`.
...
| Слот | Кирпич | Размер |
|------|--------|--------|
| Шапка | [Header Window](../components/items/header-window.md) | native OS title bar + application menu (не 48px в webview). Title: `Difference Machine` на First Start, `Difference Machine ({name})` при открытом репо |
...
| View / Project view / DFM Damaged | `6078:16278` | обзор |
...
| View / Project view / File View - File Missing | `6066:13219` | файл |
...
| `repoDamaged` | да \| нет | DFM Damaged, если `.DFM/` есть и HEAD object сломан |
...
Root / SubFolder --select folder--> тот же кадр, File Info Null
File Info --multi-select--> File More Info
* --collapse info--> Root Folder Collapse (и аналоги)
сетка --double-click folder--> SubFolder (новый folderPath)
