...
Ошибка preview (`diff.name_status`, `object.list_by_file`) и мутации (`start` / `continue` / `abort`) — `AlertBanner` внутри диалога (`title` Error). Пока Merge открыт, тот же текст в toast не дублировать. Баннер merge на окне скрыт (`hideMerge`), пока диалог открыт.
