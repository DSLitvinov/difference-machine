...
Форма создания коммита. Не модальный Dialog Settings.

Два места (один атом, разные родители):

| Сценарий | Где | Оболочка |
|----------|-----|----------|
| Create Commit / single file (`commitComposer = selection`) | **правая** колонка: File Info / Select More Files | Card Directory Selected вместо футера кнопок |
| Create Commit all files (`commitComposer = all`) | **левая** колонка: слот Uncommitted | Card Directory Selected вместо [UncommittedFilesCard](./card-directory.md) |
...
Живёт внутри [Item / Card Directory](../items/sidebar-card-directory.md) Selected. Ширина — hug родителя (справа слот 308 px, слева слот Card Directory). Stack gap 12 px. База: shadcn `Textarea` + `Button`.
