...
Открывается с [CommitProjectCard](../atoms/card-commit-project.md) / File (ellipsis). Stash ellipsis — `StashCardMenu` (Restore state / Delete), не этот popover. Пункты (checkout, revert, …) — из node; destructive — через [restore](../../dialogs/restore.md), не `window.confirm`.
