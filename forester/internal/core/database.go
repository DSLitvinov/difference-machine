...
func (d *Database) initialize() error {
	...
	if err := d.migrateScreenshotPath(); err != nil {
		...
		// Migration errors are non-critical
	}
	return nil
}
...
func (d *Database) HasChildCommits(hash string) (bool, error) {
	var count int
	err := d.db.QueryRow("SELECT COUNT(*) FROM commits WHERE parent_hash = ?", hash).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (d *Database) GetChildCommits(hash string) ([]string, error) {
	rows, err := d.db.Query("SELECT hash FROM commits WHERE parent_hash = ?", hash)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (d *Database) GetObjectsByCommit(commitHash string) ([]*models.Object, error) {
	...
	for rows.Next() {
		...
		// Parse JSON fields
		if objectDataJSON != "" {
			json.Unmarshal([]byte(objectDataJSON), &obj.ObjectData)
		} else {
			obj.ObjectData = make(map[string]interface{})
		}
