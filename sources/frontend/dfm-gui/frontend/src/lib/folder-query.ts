...
export function isDirPath(path: string, entries: DirEntry[]): boolean {
  return entries.some((entry) => entry.path === path && entry.is_dir);
}

export function fileSelection(paths: string[], entries: DirEntry[]): string[] {
  return paths.filter((path) => !isDirPath(path, entries));
}
...
export function editorLabel(absPath: string, platform = ""): string {
  const normalized = absPath.replace(/\\/g, "/");
  const parts = normalized.split("/").filter(Boolean);
  let name = parts[parts.length - 1] ?? absPath;
  if (platform === "darwin") {
