...
import { ContentViewMissing } from "@/components/items/ContentViewMissing";
...
export function ContentFilePanel({
  ...
  }, [repoPath, file.path, file.size, file.mtime, fullImage, missing]);
  ...
  }, [fullImage, missing, path, repoPath]);
  ...
  return (
    <section className="flex h-full min-w-0 flex-1 flex-col overflow-hidden pb-3 pl-2 pr-3">
      <HeaderFileAction
        ...
        collapsed={collapsed}
        locked={locked}
        ignored={Boolean(entries.find((entry) => entry.path === path)?.ignored)}
        moreDisabled={missing}
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-lg border border-border bg-background p-4 shadow-sm">
        {missing ? (
          <ContentViewMissing locale={locale} />
        ) : kind === "image" ? (
