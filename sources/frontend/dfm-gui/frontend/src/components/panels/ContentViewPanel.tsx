...
export function ContentViewPanel({ locale, folderPath, entries, onNavigate }: ContentViewPanelProps) {
  ...
  return (
    <section className="flex h-full min-w-0 flex-1 flex-col overflow-hidden pb-3 pl-2 pr-3">
      <HeaderFolderAction locale={locale} folderPath={folderPath} onNavigate={onNavigate} />
        ...
        {entries.length === 0 ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] items-start gap-2">
              ...
              {files.map((entry) => (
                <FileGridTile key={entry.path} name={entry.name} />
              ))}
            </div>
          </div>
        )}
