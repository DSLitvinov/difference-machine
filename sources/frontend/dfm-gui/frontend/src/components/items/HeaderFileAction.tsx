...
export function HeaderFileAction({ locale, fileName, collapsed, locked, ignored, moreDisabled, disableUnstage, onBack, onApply, onExpandInfo }: HeaderFileActionProps) {
  const copy = t(locale);
  return (
      ...
      <div className="flex shrink-0 items-center gap-2">
        <DropdownMenu>
          ...
          <FilePreviewItemMenu
            ...
            align="end"
            onCreateCommit={() => onApply({ kind: "createCommit" })}
            onAddInCommit={() => onApply({ kind: "addInCommit" })}
            onUnstage={() => onApply({ kind: "unstage" })}
            onIgnore={() => onApply({ kind: "ignore" })}
            onUnignore={() => onApply({ kind: "unignore" })}
