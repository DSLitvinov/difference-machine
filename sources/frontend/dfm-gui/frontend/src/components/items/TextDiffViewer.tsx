...
function DiffTabs({ value, onChange, labels }: { value: Tab; onChange: (tab: Tab) => void; labels: { unified: string; split: string } }) {
  return (
    <div className="flex items-center rounded-md bg-background-muted p-1">
      <button
        type="button"
        className={cn("rounded-sm px-3 py-1.5 text-[14px] font-medium leading-5", value === "unified" ? "bg-background text-foreground shadow-sm" : "text-foreground-muted")}
      ...
      <button
        type="button"
        className={cn("rounded-sm px-3 py-1.5 text-[14px] font-medium leading-5", value === "split" ? "bg-background text-foreground shadow-sm" : "text-foreground-muted")}
...
export function TextDiffViewer({ locale, unified }: TextDiffViewerProps) {
  const copy = t(locale);
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden">
      <div className="flex w-full items-center justify-end p-2">
        <DiffTabs value={tab} onChange={setTab} labels={{ unified: copy.tabUnified, split: copy.tabSplit }} />
      </div>
