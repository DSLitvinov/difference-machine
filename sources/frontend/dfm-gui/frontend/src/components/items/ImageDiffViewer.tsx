...
function TabBar({ value, onChange, locale }: { value: ImageTab; onChange: (tab: ImageTab) => void; locale: Locale }) {
  const copy = t(locale);
    ...
    <div className="flex items-center rounded-md bg-background-muted p-1">
      {tabs.map((tab) => (
        <button
          type="button"
          key={tab.id}
          className={cn("rounded-sm px-3 py-1.5 text-[14px] font-medium leading-5", value === tab.id ? "bg-background text-foreground shadow-sm" : "text-foreground-muted")}
...
export function ImageDiffViewer({ locale, afterSrc, beforeSrc }: ImageDiffViewerProps) {
  const copy = t(locale);
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden">
      <div className="flex w-full items-center justify-end p-2">
        <TabBar value={tab} onChange={setTab} locale={locale} />
      </div>
      {tab === "overlay" ? (
