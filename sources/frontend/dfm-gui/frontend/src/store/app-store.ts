...
export type DirEntry = {
  ...
  modified?: number;
  created?: number;
  /** True when the path is gone from disk (deleted / renamed / moved). */
  missing?: boolean;
  /** True when the path matches .dfmignore. */
  ignored?: boolean;
};
...
type AppState = {
  ...
  setViewIgnored: (value: boolean) => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  applySession: (info) => {
    ...
    set((state) => ({
      ...
      locale,
      theme,
      platform: info.platform || state.platform || "",
  ...
  setViewIgnored: (value) => set({ viewIgnored: value }),
  setFolderPath: (path) =>
  ...
  setRepoMeta: (meta) =>
    set((state) => ({
      ...meta,
      repoDamaged: meta.repoDamaged ?? Boolean(meta.status?.damaged),
      entriesToken: state.entriesToken + 1,
    })),
...
export function useDerivedView(): DerivedView {
  return useAppStore((s) =>
    deriveView({
      ...
      repoDamaged: s.repoDamaged,
      selectionCount: s.selection.length,
      fileSelectionCount: fileSelection(s.selection, s.entries).length,
