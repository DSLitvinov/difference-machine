...
func TestSetupCfgWindowSizeRoundTrip(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	if cfg.WindowWidth != 1600 || cfg.WindowHeight != 900 {
	...
	if cfg.WindowWidth != 1600 || cfg.WindowHeight != 900 {
		t.Fatalf("window size after locale write = %dx%d", cfg.WindowWidth, cfg.WindowHeight)
	}
}

func TestSetupCfgOmitsInvalidWindowSize(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	if err := updateSetupCfg(func(cfg *setupCfg) {
		cfg.WindowWidth = 640
		cfg.WindowHeight = 656
	}); err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(string(body), "window_width") || strings.Contains(string(body), "window_height") {
		t.Fatalf("invalid first-start size was persisted: %s", body)
	}
}
...
func TestSaveRepoListEmptyClearsCurrent(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	if err := rememberRepo("/Users/me/proj"); err != nil {
		t.Fatal(err)
	}
	if err := saveRepoList(nil); err != nil {
		t.Fatal(err)
	}
	state, err := loadRepoState()
	if err != nil {
	...
	if state.Current != "" || len(state.Repos) != 0 {
		t.Fatalf("empty list should clear current, got %+v", state)
	}
	body, err := os.ReadFile(filepath.Join(home, ".dfm", "repos.cfg"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(body), "[current repo]") || strings.Contains(string(body), "[repo]") {
		t.Fatalf("repos.cfg still has repo sections: %s", body)
	}
}

func TestSaveRepoListEmptyStringsClearCurrent(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	if err := rememberRepo("/Users/me/proj"); err != nil {
		t.Fatal(err)
	}
	if err := saveRepoList([]string{"", "  "}); err != nil {
		t.Fatal(err)
	}
	state, err := loadRepoState()
	if err != nil {
	...
	if state.Current != "" || len(state.Repos) != 0 {
		t.Fatalf("blank rows should clear current, got %+v", state)
	}
}

func TestSaveRepoListKeepsCurrentWhenOmittedFromNonEmptyList(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	if err := rememberRepo("/Users/me/a"); err != nil {
		t.Fatal(err)
	}
	if err := rememberRepo("/Users/me/b"); err != nil {
		t.Fatal(err)
	}
	if err := saveRepoList([]string{"/Users/me/a"}); err != nil {
		t.Fatal(err)
	}
	state, err := loadRepoState()
	if err != nil {
	...
	if state.Current != "/Users/me/b" {
		t.Fatalf("current = %q", state.Current)
	}
	if !reflect.DeepEqual(state.Repos, []string{"/Users/me/b", "/Users/me/a"}) {
		t.Fatalf("repos = %v", state.Repos)
	}
}
