...
const (
	...
	eventMenuCleanRepo     = "menu:clean-repository"
	eventMenuVerifyRepo    = "menu:verify-repository"
	eventMenuRecoverCommit = "menu:recover-commit"
)
...
func (a *App) applicationMenu() *menu.Menu {
	...
	repo.AddSeparator()
	verifyItem := repo.AddText(copy.verifyRepository, nil, func(_ *menu.CallbackData) {
		a.emitMenu(eventMenuVerifyRepo)
	})
	recoverItem := repo.AddText(copy.recoverCommit, nil, func(_ *menu.CallbackData) {
		a.emitMenu(eventMenuRecoverCommit)
	})
	repo.AddSeparator()
	cleanItem := repo.AddText(copy.cleanRepository, nil, func(_ *menu.CallbackData) {
		a.emitMenu(eventMenuCleanRepo)
	})
	...
	branches := appMenu.AddSubmenu(copy.branches)
	...
	branches.AddSeparator()
	deleteItem := branches.AddText(copy.deleteBranch, nil, func(_ *menu.CallbackData) {
		a.emitMenu(eventMenuBranchDelete)
	})

	// Linux: Wails v2 cannot replace a packed GtkMenuBar, so this snapshot is
	// permanent. Leave items enabled; webview handlers no-op without a session.
	if nativeMenuRefreshable() {
		cleanItem.Disabled = !hasSession
		verifyItem.Disabled = !hasSession
		recoverItem.Disabled = !hasSession
...
func nativeMenuRefreshable() bool {
	return goruntime.GOOS != "linux"
}

func (a *App) applyMenuLocale() {
	if a.ctx == nil || !nativeMenuRefreshable() {
		return
	}
