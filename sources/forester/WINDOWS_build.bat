...
if exist "%TARGET_FILE%" (
    ...
    REM Попытка получить версию (временный файл в build — путь всегда валидный)
    set "VER_TMP=%BUILD_DIR%\forester_ver.txt"
    "%TARGET_FILE%" --version > "%VER_TMP%" 2>nul
    if errorlevel 1 (
        if exist "%VER_TMP%" del /q "%VER_TMP%" >nul 2>&1
        ...
        if exist "%VER_TMP%" (
            for /f "usebackq delims=" %%i in ("%VER_TMP%") do echo [OK] Версия: %%i
            del /q "%VER_TMP%" >nul 2>&1
        ) else (
            echo [OK] Бинарник работает
        )
    )
) else (
