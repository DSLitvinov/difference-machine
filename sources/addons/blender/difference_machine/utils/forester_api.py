...
class ForesterAPIWrapper:
    ...
    def commit(self, repo_path: Path, message: str, author: Optional[str] = None, tag: Optional[str] = None) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        if self._use_api and self._api_instance:
            try:
                result = self._api_instance.commit(str(repo_path), message, author)
                if result.get("success"):
                    # API doesn't return commit hash directly, need to get it from log
                    # For now, return success without hash
                    commit_hash = None
                    try:
    ...
    def log(self, repo_path: Path, branch: Optional[str] = None, limit: int = 100) -> Tuple[bool, List[Dict[str, Any]], Optional[str]]:
        """Get commit log."""
        if self._use_api and self._api_instance:
            try:
                # Validate repo_path before calling C API
                if not repo_path or not repo_path.exists():
                    return False, [], f"Repository path does not exist: {repo_path}"
                
                commits = self._api_instance.get_log(str(repo_path), max_count=limit if limit > 0 else 0, branch=branch)
                
                # Check for None result (C API failure)
                if commits is None:
                    logger.warning(f"get_log returned None for repo_path={repo_path}, branch={branch}")
                ...
                try:
                    for commit in commits:
                        # Validate commit object before accessing attributes
