...
class ForesterAPI:
    ...
    def _setup_function_signatures(self):
        ...
        if hasattr(self.lib, "ForesterRestoreVersion"):
            self.lib.ForesterRestoreVersion.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
            self.lib.ForesterRestoreVersion.restype = ctypes.POINTER(ForesterResult)
    ...
    def restore_version(self, repo_path: str, commit_hash: str) -> Dict[str, Any]:
        ...
        if not hasattr(self.lib, "ForesterRestoreVersion"):
            return {"success": False, "error": "ForesterRestoreVersion not available"}
        result_ptr = self.lib.ForesterRestoreVersion(
            repo_path.encode('utf-8'),
            commit_hash.encode('utf-8'),
        )
        if not result_ptr:
