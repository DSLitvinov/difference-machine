	...
	for i, commit := range commits {
		...
		if showStat || nameOnly || nameStatus {
			if commit.ParentHash != "" {
				added, modified, deleted, renamed = getCommitFileChanges(storage, repo, commit)
			} else {
		...
		if oneline || prettyFormat == "oneline" {
			...
			if showStat {
				// Show statistics
				totalFiles := len(added) + len(modified) + len(deleted) + len(renamed)
				if totalFiles > 0 {
					...
					if len(renamed) > 0 {
						fmt.Printf(", %d renamed", len(renamed))
					}
					if len(deleted) > 0 {
				...
				for _, pair := range renamed {
					allFiles = append(allFiles, pair.NewPath)
				}
...
// getCommitFileChanges returns added, modified, deleted, and renamed files in a commit.
func getCommitFileChanges(storage *core.Storage, repo *core.Repository, commit *models.Commit) (added, modified, deleted []string, renamed []renamePair) {
	if commit.ParentHash == "" {
		return
	}
	...
	renamed, added, deleted = pairRenamesByHash(added, deleted, func(path string) (string, bool) {
		if entry, ok := commitTreeMap[path]; ok && entry.Type == "blob" {
			return entry.Hash, true
		}
		if entry, ok := parentTreeMap[path]; ok && entry.Type == "blob" {
			return entry.Hash, true
		}
		return "", false
	})
