...
func Diff(args []string) error {
	...
	renamed, added, deleted := pairRenamesByHash(added, deleted, func(path string) (string, bool) {
		if entry, ok := tree2Map[path]; ok {
			return entry.Hash, true
		}
		if entry, ok := tree1Map[path]; ok {
			return entry.Hash, true
		}
		return "", false
	})
	...
	// Print differences
	if len(added) == 0 && len(modified) == 0 && len(deleted) == 0 && len(renamed) == 0 {
	...
	if len(renamed) > 0 {
		fmt.Println(utils.Green("Renamed files:"))
		for _, pair := range renamed {
