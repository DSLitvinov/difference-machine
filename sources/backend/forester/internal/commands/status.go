...
func Status(args []string) error {
	...
	workdirHash := make(map[string]string)
	...
	for _, filePath := range allFiles {
		...
		workdirHash[normalizedPath] = currentHash
	...
	hashOf := func(path string) (string, bool) {
		...
		if hash, ok := normalizedIndexMap[normalized]; ok && !core.IsDeletedHash(hash) {
			return hash, true
		}
		if hash, ok := workdirHash[normalized]; ok {
			return hash, true
		}
		hash, ok := trackedMap[normalized]
		return hash, ok
	}
	allDeleted := append(append([]string{}, stagedDeletedFiles...), unstagedDeletedFiles...)
	renamedFiles, stagedNewFiles, _ := pairRenamesByHash(stagedNewFiles, allDeleted, hashOf)
	droppedOld := droppedOldPaths(renamedFiles)
	remainingDeleted := rejectPaths(allDeleted, droppedOld)
	moreRenamed, untrackedFiles, _ := pairRenamesByHash(untrackedFiles, remainingDeleted, hashOf)
	renamedFiles = append(renamedFiles, moreRenamed...)
	droppedOld = droppedOldPaths(renamedFiles)
	stagedDeletedFiles = rejectPaths(stagedDeletedFiles, droppedOld)
	unstagedDeletedFiles = rejectPaths(unstagedDeletedFiles, droppedOld)

	hasStaged := len(stagedNewFiles) > 0 || len(stagedModifiedFiles) > 0 || len(stagedDeletedFiles) > 0 || len(renamedFiles) > 0
	...
	if shortFormat {
		...
		for _, pair := range renamedFiles {
			allFiles[formatRename(pair)] = "R "
		}
	...
	if hasStaged {
		...
		if len(renamedFiles) > 0 {
			for _, pair := range renamedFiles {
				fmt.Printf("  %s %s\n", utils.Green("R"), formatRename(pair))
			}
		}
