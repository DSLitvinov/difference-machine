...
func handleIndexDrop(workPath string, args json.RawMessage) (interface{}, error) {
	var params struct {
		Files []string `json:"files"`
	}
	_ = decodeArgs(args, &params)
	...
	cmdArgs := append([]string{"--staged"}, params.Files...)
	_, err := withWorkDir(workPath, func() (interface{}, error) {
...
func computeStatus(repo *core.Repository, repoPath string) (map[string]interface{}, error) {
	...
	untrackedHash := map[string]string{}
	if (len(stagedDeleted) > 0 || len(unstagedDeleted) > 0) && len(untracked) > 0 {
		for _, relPath := range untracked {
			...
			untrackedHash[relPath] = hash
		}
	}

	renamedFiles, stagedNew, stagedDeleted, unstagedDeleted, untracked := detectWorkingTreeRenames(
		stagedNew, stagedDeleted, unstagedDeleted, untracked, indexMap, trackedMap, untrackedHash,
	)
...
func detectWorkingTreeRenames(
	stagedNew, stagedDeleted, unstagedDeleted, untracked []string,
	indexMap, trackedMap, untrackedHash map[string]string,
	...
	hashOfNew := func(path string) (string, bool) {
		if hash, ok := indexMap[path]; ok && !core.IsDeletedHash(hash) {
			return hash, true
		}
		hash, ok := untrackedHash[path]
		return hash, ok
	}

	pairNew := func(newPaths []string) []string {
		remaining := make([]string, 0, len(newPaths))
		for _, newPath := range newPaths {
			hash, ok := hashOfNew(newPath)
			if !ok {
	...
	remainingNew := pairNew(stagedNew)
	remainingUntracked := pairNew(untracked)
	...
	return renamedFiles, remainingNew, filterDeleted(stagedDeleted), filterDeleted(unstagedDeleted), remainingUntracked
}
