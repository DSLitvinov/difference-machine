...
func TestStatusDamagedWhenHeadObjectMissing(t *testing.T) {
	dir, h := initTestRepo(t)
	writeFile(t, dir, "keep.txt", "ok")
	mustOK(t, h, "index.add", `{"files":["keep.txt"]}`)
	mustOK(t, h, "commit.create", `{"message":"seed"}`)
	...
	if status.Head == "" || status.Damaged {
		t.Fatalf("healthy status = %+v", status)
	}

	hash := status.Head
	obj := filepath.Join(dir, ".DFM", "objects", hash[:2], hash[2:])
	if err := os.Remove(obj); err != nil {
		t.Fatal(err)
	}

	if err := json.Unmarshal(mustOK(t, h, "status.get", `{}`), &status); err != nil {
		t.Fatalf("decode damaged status: %v", err)
	}
	if !status.Damaged {
		t.Fatal("expected damaged=true after removing HEAD object")
	}

	var rebuild struct {
		Damaged bool `json:"damaged"`
	}
	if err := json.Unmarshal(mustOK(t, h, "repo.rebuild", `{}`), &rebuild); err != nil {
		t.Fatalf("decode rebuild: %v", err)
	}
	if !rebuild.Damaged {
		t.Fatal("expected rebuild damaged=true")
	}
}
...
func TestReflogGetAndRestore(t *testing.T) {
	...
	second := logResult.Commits[0].Hash

	mustOK(t, h, "commit.reset", fmt.Sprintf(`{"commit_hash":%q,"mode":"mixed"}`, first))
	...
	if err := json.Unmarshal(mustOK(t, h, "reflog.get", `{"limit":50}`), &reflog); err != nil {
		t.Fatalf("decode reflog: %v", err)
	}
	if len(reflog.Entries) == 0 {
		t.Fatal("expected reflog entries after reset")
	}

	mustOK(t, h, "reflog.restore", fmt.Sprintf(`{"commit_hash":%q}`, second))
	if err := json.Unmarshal(mustOK(t, h, "log.get", `{}`), &logResult); err != nil {
		t.Fatalf("decode log after restore: %v", err)
	}
...
func TestIndexDropUnstage(t *testing.T) {
	dir, h := initTestRepo(t)
	writeFile(t, dir, "new.txt", "new")
	mustOK(t, h, "index.add", `{"files":["new.txt"]}`)

	var staged struct {
		StagedNew []string `json:"staged_new_files"`
		Untracked []string `json:"untracked_files"`
	}
	if err := json.Unmarshal(mustOK(t, h, "status.get", `{}`), &staged); err != nil {
		t.Fatalf("decode status: %v", err)
	}
	if len(staged.StagedNew) != 1 || staged.StagedNew[0] != "new.txt" {
		t.Fatalf("staged_new = %v", staged.StagedNew)
	}

	mustOK(t, h, "index.drop", `{"files":["new.txt"]}`)
	if err := json.Unmarshal(mustOK(t, h, "status.get", `{}`), &staged); err != nil {
		t.Fatalf("decode status: %v", err)
	}
	if len(staged.StagedNew) != 0 {
		t.Fatalf("staged_new after drop = %v", staged.StagedNew)
	}
	foundNew := false
	for _, f := range staged.Untracked {
		if f == "new.txt" {
			foundNew = true
			break
		}
	...
	if !foundNew {
		t.Fatalf("untracked = %v, want new.txt", staged.Untracked)
	}
	...
	if err := json.Unmarshal(mustOK(t, h, "status.get", `{}`), &modified); err != nil {
		t.Fatalf("decode status: %v", err)
	}
	if len(modified.StagedModified) != 1 || modified.StagedModified[0] != "tracked.txt" {
		t.Fatalf("staged_modified = %v", modified.StagedModified)
	}

	mustOK(t, h, "index.drop", `{"files":["tracked.txt"]}`)
	if err := json.Unmarshal(mustOK(t, h, "status.get", `{}`), &modified); err != nil {
		t.Fatalf("decode status: %v", err)
	}
	if len(modified.StagedModified) != 0 {
		t.Fatalf("staged_modified after drop = %v", modified.StagedModified)
	}
	if len(modified.UnstagedModified) != 1 || modified.UnstagedModified[0] != "tracked.txt" {
		t.Fatalf("unstaged_modified after drop = %v", modified.UnstagedModified)
	}

	mustFail(t, h, "index.drop", `{}`)
	mustFail(t, h, "index.drop", `{"files":[]}`)
}
