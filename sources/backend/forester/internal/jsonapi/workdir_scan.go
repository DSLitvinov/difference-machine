...
func (s *workdirScanner) isInternalName(name string, rel string) bool {
	...
	return rel == ".dfmignore"
}

func (s *workdirScanner) isIgnoredRel(rel string, isDir bool) bool {
	rel = filepath.ToSlash(rel)
	if rel == "" || rel == "." {
		return false
	}
	...
	return isDir && s.patterns.Matches(rel+"/")
}

func (s *workdirScanner) shouldSkipName(name string, rel string, isDir bool) bool {
	if s.isInternalName(name, rel) {
		return true
	}
	return s.isIgnoredRel(rel, isDir)
}
...
// Empty rel opens the repository root (Open in folder for a root file).
func (s *workdirScanner) absOpenPath(rel string) (string, error) {
	...
	if rel == "" {
		return s.ensureInsideRepo(s.repoPath)
	}
	fileAbs, fileErr := s.absFile(rel)
	if fileErr == nil {
		return fileAbs, nil
	}
	dirAbs, dirErr := s.absDir(rel)
	if dirErr != nil {
		return "", fileErr
	}
	...
	return s.ensureInsideRepo(dirAbs)
}
...
func (s *workdirScanner) countFilesRecursive(rel string, includeIgnored bool) (int, error) {
	...
	err = filepath.Walk(absDir, func(path string, fi os.FileInfo, walkErr error) error {
		...
		if fi.IsDir() {
			if s.skipEntry(fi.Name(), relPath, true, includeIgnored) {
				return filepath.SkipDir
			}
			return nil
		}
		if s.skipEntry(fi.Name(), relPath, false, includeIgnored) {
			return nil
		}
		count++
		return nil
...
func (s *workdirScanner) skipEntry(name, rel string, isDir, includeIgnored bool) bool {
	if s.isInternalName(name, rel) {
		return true
	}
	return !includeIgnored && s.isIgnoredRel(rel, isDir)
}
...
func (s *workdirScanner) listFolderChildren(rel string, depth int) ([]folderNode, error) {
	...
	for _, entry := range entries {
		...
		itemCount, err := s.countFilesRecursive(childRel, false)
		if err != nil {
			return nil, err
		}
...
func (s *workdirScanner) listEntries(rel string, includeIgnored bool) ([]dirEntry, error) {
	absDir, err := s.absDir(rel)
	if err != nil {
		return nil, err
	}
	...
	for _, entry := range entries {
		...
		isDir := entry.IsDir()
		if s.skipEntry(name, childRel, isDir, includeIgnored) {
			continue
		}
		...
		item := dirEntry{
			...
			Path:    childRel,
			IsDir:   isDir,
			Ignored: s.isIgnoredRel(childRel, isDir),
		}
		if isDir {
			count, err := s.countFilesRecursive(childRel, includeIgnored)
			if err != nil {
				return nil, err
			}
...
func (s *workdirScanner) treeNode(rel string, depth int) (folderNode, error) {
	...
	itemCount, err := s.countFilesRecursive(rel, false)
	if err != nil {
		return folderNode{}, err
	}
...
func (s *workdirScanner) search(query string, limit int, includeIgnored bool) ([]dirEntry, bool, error) {
	...
	err := filepath.Walk(s.repoPath, func(path string, fi os.FileInfo, walkErr error) error {
		...
		isDir := fi.IsDir()
		if s.skipEntry(fi.Name(), relPath, isDir, includeIgnored) {
			if isDir {
				return filepath.SkipDir
			}
			return nil
		...
		item := dirEntry{
			...
			Path:    relPath,
			IsDir:   isDir,
			Ignored: s.isIgnoredRel(relPath, isDir),
		}
		if isDir {
			count, err := s.countFilesRecursive(relPath, includeIgnored)
			if err != nil {
				return err
			}
			item.ItemCount = count
...
func (s *workdirScanner) entriesForPaths(paths []string) ([]dirEntry, error) {
	...
	for _, rel := range paths {
		...
		item := dirEntry{
			...
			IsDir:   false,
			Size:    info.Size(),
			Ignored: s.isIgnoredRel(rel, false),
		}
		fillEntryTimestamps(&item, info)
		out = append(out, item)
	}
