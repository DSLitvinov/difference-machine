...
func handleWorkdirSearch(workPath string, args json.RawMessage) (interface{}, error) {
	...
	return withRepo(workPath, func(_ *core.Repository, repoPath string) (interface{}, error) {
		scanner := newWorkdirScanner(repoPath)
		entries, capped, err := scanner.search(params.Query, limit, params.IncludeIgnored)
		if err != nil {
...
func handleWorkdirDfmignoreGet(workPath string, args json.RawMessage) (interface{}, error) {
	_ = args
	return withRepo(workPath, func(_ *core.Repository, repoPath string) (interface{}, error) {
		content, err := utils.ReadDfmignore(repoPath)
		if err != nil {
			return nil, err
		}
...
func handleWorkdirDfmignoreSet(workPath string, args json.RawMessage) (interface{}, error) {
	var params struct {
	...
	return withRepo(workPath, func(_ *core.Repository, repoPath string) (interface{}, error) {
		if err := utils.WriteDfmignore(repoPath, params.Content); err != nil {
			return nil, err
		}
		return successResult(), nil
	})
...
func handleWorkdirIgnore(workPath string, args json.RawMessage) (interface{}, error) {
	return mutateDfmignore(workPath, args, utils.AppendDfmignorePatterns)
}

func handleWorkdirUnignore(workPath string, args json.RawMessage) (interface{}, error) {
	return mutateDfmignore(workPath, args, utils.RemoveDfmignorePatterns)
}

func mutateDfmignore(workPath string, args json.RawMessage, write func(repoPath string, patterns []string) error) (interface{}, error) {
	var params struct {
		Paths []string `json:"paths"`
	}
	if err := decodeArgs(args, &params); err != nil {
	...
	return withRepo(workPath, func(_ *core.Repository, repoPath string) (interface{}, error) {
		scanner := newWorkdirScanner(repoPath)
		patterns := make([]string, 0, len(params.Paths))
		for _, path := range params.Paths {
			rel := canonicalRelPath(path)
			if rel == "" {
				return nil, fmt.Errorf("path is required")
			}
			name := filepath.Base(filepath.FromSlash(rel))
			if scanner.isInternalName(name, rel) {
				return nil, fmt.Errorf("path is not accessible")
			}
			parentAbs, err := scanner.absDir(parentRepoRelPath(rel))
			if err != nil {
				return nil, err
			}
			abs := filepath.Join(parentAbs, name)
			abs, err = scanner.ensureInsideRepo(abs)
			if err != nil {
				return nil, err
			}
			info, err := os.Stat(abs)
			...
			pattern := utils.IgnorePatternForRel(rel, info.IsDir())
			if pattern == "" {
				return nil, fmt.Errorf("path is required")
			}
			patterns = append(patterns, pattern)
		}
		if err := write(repoPath, patterns); err != nil {
			return nil, err
		}
		return successResult(), nil
	})
...
func isImageExt(ext string) bool {
	switch ext {
	case ".png", ".jpg", ".jpeg", ".gif", ".webp", ".exr", ".tiff", ".tif", ".bmp", ".svg":
		return true
	default:
		return false
	}
...
func isTextExt(ext string) bool {
	switch ext {
	case ".txt", ".md", ".markdown", ".json", ".xml", ".tsx", ".ts", ".js", ".jsx", ".mjs", ".cjs",
