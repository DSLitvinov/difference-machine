...
func TestStatusGetRenameAfterWorkdirRename(t *testing.T) {
	dir, h := initTestRepo(t)
	writeFile(t, dir, "dir/old.txt", "payload")
	mustOK(t, h, "index.add", `{"files":["dir/old.txt"]}`)
	mustOK(t, h, "commit.create", `{"message":"seed","author":"tester"}`)

	raw := mustOK(t, h, "workdir.rename", `{"path":"dir/old.txt","new_name":"new.txt"}`)
	var renamed struct {
	...
	if renamed.NewPath != "dir/new.txt" {
		t.Fatalf("new_path = %q", renamed.NewPath)
	}
	...
	if len(statusPayload.UntrackedFiles) != 0 {
		t.Fatalf("untracked_files = %v, want empty after rename pairing", statusPayload.UntrackedFiles)
	}
	if len(statusPayload.UnstagedDeleted) != 0 {
		t.Fatalf("unstaged_deleted_files = %v, want empty after rename pairing", statusPayload.UnstagedDeleted)
	}
