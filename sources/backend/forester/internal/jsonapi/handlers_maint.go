...
func handleRepoRebuild(workPath string, _ json.RawMessage) (interface{}, error) {
	return withRepo(workPath, func(repo *core.Repository, repoPath string) (interface{}, error) {
		...
		head := ""
		detached, state, detErr := core.ReadDetachedHead(repoPath)
		if detErr == nil && detached {
			head = state.Commit
		} else {
			name, _ := repo.Refs.GetCurrentBranch()
			if name != "" {
				head, _ = repo.GetBranchHead(name)
			}
		}
		...
		return map[string]interface{}{
			...
			"damaged":         headObjectsBroken(repo, head),
		}, nil
	})
}
