...
def main():
    ...
    # Use Fusion style (built-in Qt Quick Controls) instead of Basic
    ...
    # Translation manager needs engine for retranslate on language change
    translation_manager.setEngine(engine)
    
    # Singleton-like instances for QML
    ...
    # Sync FileManager with repository from history (so file tree loads on startup)
    repository_manager.repositoryChanged.connect(file_manager.setDirectory)
    if repository_manager.currentRepository:
        file_manager.setDirectory(repository_manager.currentRepository)
    ...
    # Connect language change to retranslate UI (uses engine set via setEngine)
    def onLanguageChanged(lang_code):
        translation_manager.retranslate()
