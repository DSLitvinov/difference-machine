...
class TranslationManager(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._app = None
        self._engine = None  # Set from main.py for retranslate on language change
    ...
    def setEngine(self, engine):
        """Set the QML engine for retranslate (called from main.py)."""
        self._engine = engine
    ...
    @pyqtSlot('QObject')
    def retranslate(self, engine=None):
        """Trigger retranslation of QML UI. Can be called with no args (uses engine set via setEngine) or with engine from QML."""
        target = engine if engine is not None else self._engine
        if target and hasattr(target, "retranslate"):
            target.retranslate()
    ...
    def setLanguage(self, language_code):
        ...
        translation_file = self._translations_path / f"difference_machine_{language_code}.qm"
        ...
        if translation_file.exists():
            ...
            if self._translator.load(file_path):
                ...
                if self._translator.load(current_locale, "difference_machine", "_", str(self._translations_path), ".qm"):
                    self._app.installTranslator(self._translator)
                    self._current_language = language_code
                    self.languageChanged.emit(language_code)
                    return True
