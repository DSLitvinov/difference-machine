...
class RepositoryManager(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._load_recent_repositories()
    ...
    @pyqtProperty(str, notify=defaultEmailChanged)
    ...
    def _recent_repos_path(self) -> Path:
        """Path to repository list config file."""
        return Path.home() / ".dfm" / "repo_list.cfg"

    def _load_recent_repositories(self) -> None:
        """Load recent repository paths from ~/.dfm/repo_list.cfg. Creates file if missing."""
        path = self._recent_repos_path()
        if not path.exists():
            try:
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text("", encoding="utf-8")
            except Exception as e:
                log.warning("Failed to create repo_list.cfg: %s", e)
            self._recent_repositories = []
            return
        repos: List[str] = []
        ...
        try:
            with open(path, encoding="utf-8") as f:
                content = f.read()
            block_start = "[name repository]"
            for block in content.split(block_start):
                block = block.strip()
                if not block:
                    continue
                path_val = ""
                ...
                for line in block.splitlines():
                    ...
                    if line.startswith("path"):
                        eq = line.find("=")
                        if eq >= 0:
                            path_val = line[eq + 1:].strip()
                    elif line.startswith("active"):
                        eq = line.find("=")
                        if eq >= 0:
                            active_val = line[eq + 1:].strip().lower() in ("true", "1", "yes")
                if path_val and path_val not in repos:
                    resolved = str(Path(path_val).resolve())
                    if Path(resolved).is_dir():
                        repos.append(resolved)
                        if active_val:
                            active_path = resolved
            self._recent_repositories = repos
            if active_path and active_path != self._current_repository:
                self._current_repository = active_path
                _images_module.clear_image_cache()
                self.repositoryChanged.emit(active_path)
                self._refresh_status()
            self.recentRepositoriesChanged.emit()
        except Exception as e:
            log.warning("Failed to load repo_list.cfg: %s", e)
            self._recent_repositories = []

    def _save_recent_repositories(self) -> None:
        """Save repository list to ~/.dfm/repo_list.cfg in [name repository] blocks."""
        path = self._recent_repos_path()
        try:
            path.parent.mkdir(parents=True, exist_ok=True)
            lines: List[str] = []
            for i, repo_path in enumerate(self._recent_repositories):
                if i > 0:
                    lines.append("")
                active = (repo_path == self._current_repository)
                lines.append("[name repository]")
                lines.append("active = " + ("true" if active else "false"))
                lines.append("path = " + repo_path)
            with open(path, "w", encoding="utf-8") as f:
                f.write("\n".join(lines))
                if lines:
                    f.write("\n")
        except Exception as e:
            log.warning("Failed to save repo_list.cfg: %s", e)

    @pyqtProperty('QVariant', notify=recentRepositoriesChanged)
    def recentRepositories(self):
        """List of recent repository paths (most recent first)."""
        return self._recent_repositories

    @pyqtSlot(str)
    def addRecentRepository(self, repo_path: str):
        """Add path to recent list, move to front, persist."""
        ...
        try:
            ...
            if not Path(resolved).is_dir():
                return
            if resolved in self._recent_repositories:
                self._recent_repositories.remove(resolved)
            self._recent_repositories.insert(0, resolved)
            self._recent_repositories = self._recent_repositories[: self._MAX_RECENT_REPOS]
            self._save_recent_repositories()
            self.recentRepositoriesChanged.emit()
        except Exception as e:
            log.warning("addRecentRepository failed: %s", e)

    @pyqtSlot(str)
    def removeRecentRepository(self, repo_path: str):
        """Remove path from recent list and update repo_list.cfg."""
        if not repo_path or not repo_path.strip():
            return
        try:
            resolved = str(Path(repo_path).resolve())
            if resolved in self._recent_repositories:
                self._recent_repositories.remove(resolved)
                if self._current_repository == resolved:
                    self._current_repository = ""
                    _images_module.clear_image_cache()
            ...
            log.warning("removeRecentRepository failed: %s", e)
    ...
    def setRepository(self, repo_path: str):
        ...
        if repo_path and repo_path.strip():
            self.addRecentRepository(repo_path)
    ...
    def getLog(self, max_count: int = 0, branch: Optional[str] = None):
        ...
        try:
            ...
            self._set_error(f"Error getting log: {e}")
    ...
    def getBranches(self):
        ...
        try:
            ...
            self._set_error(f"Error getting branches: {e}")
    ...
    def createCommit(self, message: str, author: str = "", email: str = "", tag: str = "") -> bool:
        """Create a commit with optional author, email, and tag."""
        if not self._current_repository or not self._api_wrapper:
            self._set_error("Repository path not set or API unavailable.")
            return False
        if not message or not message.strip():
            ...
            return False
        # Combine author and email in Git format: "Name <email@example.com>"
        author_str = None
        if author or email:
            author_trimmed = author.strip() if author else ""
            email_trimmed = email.strip() if email else ""
            if author_trimmed and email_trimmed:
                author_str = f"{author_trimmed} <{email_trimmed}>"
            elif author_trimmed:
                author_str = author_trimmed
            elif email_trimmed:
                author_str = f"<{email_trimmed}>"
        try:
    ...
    @pyqtSlot(str, result=bool)
    def restoreVersion(self, commit_hash: str) -> bool:
        """Restore working directory to the given commit (full overwrite), then add and create a commit."""
        if not self._current_repository or not self._api_wrapper:
            self._set_error("Repository path not set or API unavailable.")
            return False
        if not commit_hash or not commit_hash.strip():
        ...
        h = commit_hash.strip()
        try:
            ok, error = self._api_wrapper.restore_version(repo, h)
            if not ok:
                if error:
                    self._set_error(error)
                return False
            ok, error = self._api_wrapper.add(repo, ["."])
            if not ok:
                self._set_error(error or "Failed to add files after restore.")
                return False
            author_str = None
            author_trimmed = (get_user_name() or "").strip()
            email_trimmed = (get_user_email() or "").strip()
            if author_trimmed and email_trimmed:
                author_str = f"{author_trimmed} <{email_trimmed}>"
            elif author_trimmed:
                author_str = author_trimmed
            ...
            message = f"Restore version {msg_time} from commit {short_hash}"
            ok, _, error = self._api_wrapper.create_commit(repo, message, author_str, None)
            if not ok:
                self._set_error(error or "Restore done but commit failed.")
                return False
            self._clear_error()
            self._refresh_status()
            return True
        except Exception as e:
            self._set_error(f"Restore failed: {e}")
        return False
    ...
    def getCommit(self, commit_hash: str):
        ...
        try:
            ok, commit, error = self._api_wrapper.get_commit(Path(self._current_repository), commit_hash)
            if ok and commit:
                return commit
            if error:
                self._set_error(error)
        except Exception as e:
            self._set_error(f"Error getting commit: {e}")
    ...
    def compareCommitFile(self, commit_hash: str, file_path: str) -> bool:
        ...
        def write_temp_file(target_hash: str, label: str) -> Optional[Path]:
            ...
            temp_dir = Path(tempfile.mkdtemp(prefix=f"difference_machine_compare_{label}_"))
