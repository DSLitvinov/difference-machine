...
GUI_SRC_DIR="${INSTALLER_DIR}/${PLATFORM_DIR}/diffmachine_gui"
GUI_INSTALL_DIR="${INSTALL_PATH}/diffmachine_gui"
...
# Install GUI (diffmachine_gui): PyInstaller-сборка или исходники + venv
GUI_INSTALLED=false
GUI_LAUNCHER=""
...
if [ -d "${GUI_SRC_DIR}" ]; then
    ...
    # Сборка PyInstaller: исполняемый файл DifferenceMachine в корне diffmachine_gui
                ...
                if [ -n "${BLENDER_VERSIONS}" ]; then
                    ...
                    if [[ "${INSTALL_ALL}" =~ ^[Yy]$ ]]; then
                        for BLENDER_VERSION in ${BLENDER_VERSIONS}; do
                            ADDON_DEST="${BLENDER_VERSION}/extensions/user_default/diffmachine"
                            mkdir -p "${ADDON_DEST}"
                            cp -r "${INSTALLER_DIR}/addons/blender/diffmachine"/* "${ADDON_DEST}/" 2>/dev/null || {
                        ...
                        ADDON_DEST="${BLENDER_ADDON_PATH}/${BLENDER_VERSION}/extensions/user_default/diffmachine"
                        if [ -d "${BLENDER_ADDON_PATH}/${BLENDER_VERSION}" ]; then
                            mkdir -p "${ADDON_DEST}"
                            cp -r "${INSTALLER_DIR}/addons/blender/diffmachine"/* "${ADDON_DEST}/"
                    ...
                    if [ -d "${CUSTOM_BLENDER_PATH}" ]; then
                        ADDON_DEST="${CUSTOM_BLENDER_PATH}/diffmachine"
                        mkdir -p "${ADDON_DEST}"
                        cp -r "${INSTALLER_DIR}/addons/blender/diffmachine"/* "${ADDON_DEST}/"
