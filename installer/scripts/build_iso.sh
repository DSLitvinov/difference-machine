...
# Ожидает: DFM_Installer/ уже собран (build_installer.sh или build_installer_full.sh).
...
if [ ! -d "${DFM_INSTALLER_DIR}" ]; then
    echo -e "${YELLOW}⚠ Папка ${DFM_INSTALLER_DIR} не найдена. Сначала запустите build_installer.sh или build_installer_full.sh.${NC}"
    exit 1
fi
